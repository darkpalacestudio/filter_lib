...
#include <filter_lib/so_apf.hpp>
#include <Approvals.h>

TEST_CASE("SO APF does not affect intensity of any frequency")
{
    SO_APF filter;
    ...
    for (auto const filter_frequency : { 10.0f, 100.0f, 1000.0f, 10000.0f }) {
            ...
            + test_helpers::floatToStringWithXDecimalDigits(filter_frequency, 0))
        {
            ...
            REQUIRE(std::abs(max - 1.0f) < epsilon);
            REQUIRE(std::abs(min + 1.0f) < epsilon);
        }
    }
}
